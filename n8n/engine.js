...
function rowsToRawObjects(grid) {
  ...
  // keyed by the mangled header; "" cells become null (pandas .replace("", NA)).
  if (!grid || grid.length === 0) throw new Error("empty sheet -- no header row");
  const width = Math.max.apply(null, grid.map((r) => r.length));
  const padded = grid.map((r) => r.concat(Array(width - r.length).fill("")));
  const headerIdx = findHeaderRow(padded);
  const header = resolveHeader(padded[headerIdx]);
  return padded.slice(headerIdx + 1).map((r) => {
    const obj = {};
